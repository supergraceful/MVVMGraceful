...
public final class ToastUtils {
    ...
    private static int yOffset = (int) (64 * UtilsBridge.getApplication().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) UtilsBridge.getApplication().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(UtilsBridge.getApplication().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(UtilsBridge.getApplication().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(UtilsBridge.getApplication());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(UtilsBridge.getApplication(), spannableString, duration);
            } else {
                sToast = Toast.makeText(UtilsBridge.getApplication(), text, duration);
            }
        }
